...
export default function NotificationsPage() {
  const { suitablePositions, setSuitablePositions, cvDatabase } =
    useAppContext();
  const [selectedCandidates, setSelectedCandidates] = useState<
    Record<string, Set<string>>
  >({});
  const [loadingAssessments, setLoadingAssessments] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleQuickAddToAssessment = useCallback(
    (positions: SuitablePosition[]) => {
      ...
      const handledKeys = new Set(
        positions.map((p) => `${p.candidateEmail}-${p.assessment.id}`)
      );
      setSuitablePositions((prev) =>
        prev.filter(
          (p) => !handledKeys.has(`${p.candidateEmail}-${p.assessment.id}`)
        )
      );
  ...
  const handleBulkAdd = useCallback(
    async (assessmentId: string, candidatesInGroup: SuitablePosition[]) => {
      const selectedEmails = selectedCandidates[assessmentId] || new Set();
      if (selectedEmails.size === 0) return;

      const positionsToAdd = candidatesInGroup.filter((p) =>
        selectedEmails.has(p.candidateEmail)
      );
  ...
  const handleToggleAll = (
    assessmentId: string,
    candidatesInGroup: SuitablePosition[],
    allSelected: boolean
  ) => {
    setSelectedCandidates((prev) => {
      const newSelections = { ...prev };
      if (allSelected) {
        ...
        newSelections[assessmentId] = new Set(
          candidatesInGroup.map((c) => c.candidateEmail)
        );
      }
      return newSelections;
    });
  ...
  const groupedPositions = useMemo(() => {
    return suitablePositions.reduce(
      (acc, pos) => {
      ...
      {} as Record<
        string,
        { assessmentInfo: AssessmentSession; candidates: SuitablePosition[] }
      >
    );
  }, [suitablePositions]);

  const groupedPositionsArray = Object.values(groupedPositions).sort(
    (a, b) =>
      new Date(b.assessmentInfo.createdAt).getTime() -
      new Date(a.assessmentInfo.createdAt).getTime()
  );
          ...
          <Card className="shadow-sm">
            <CardHeader className="flex flex-row justify-between items-center border-b">
              <div>
                ...
                <CardDescription>
                  New relevant roles found for candidates in your database.
                </CardDescription>
              </div>
            ...
            <CardContent className="p-0">
              {groupedPositionsArray.length > 0 ? (
                <div className="divide-y">
                  {groupedPositionsArray.map(
                    ({ assessmentInfo, candidates }) => {
                      const selectedForGroup =
                        selectedCandidates[assessmentInfo.id] || new Set();
                      const allSelectedInGroup =
                        selectedForGroup.size === candidates.length &&
                        candidates.length > 0;
                      const isLoading = loadingAssessments.has(
                        assessmentInfo.id
                      );

                      return (
                        <div
                          key={assessmentInfo.id}
                        ...
                        >
                          <div className="flex justify-between items-center gap-4">
                            ...
                            >
                              <h5
                                ...
                                title={assessmentInfo.analyzedJd.JobTitle}
                              >
                                {assessmentInfo.analyzedJd.JobTitle}
                              </h5>
                              <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                                <Users className="h-4 w-4" />{' '}
                                {candidates.length} new suitable candidate(s)
                              </p>
                            </Link>
                            <Button
                              size="sm"
                              disabled={
                                selectedForGroup.size === 0 || isLoading
                              }
                              onClick={() =>
                                handleBulkAdd(assessmentInfo.id, candidates)
                              }
                            >
                              {isLoading ? (
                          ...
                          <div className="pl-4 mt-4 space-y-2 border-l">
                            <div className="flex items-center gap-3 text-sm text-muted-foreground">
                              ...
                              />
                              <label
                                htmlFor={`select-all-${assessmentInfo.id}`}
                            ...
                            {candidates.map((pos) => (
                              ...
                              >
                                <div className="flex items-center gap-3 flex-1 overflow-hidden">
                                  ...
                                  />
                                  <Link
                                    href={`/cv-database?email=${encodeURIComponent(pos.candidateEmail)}`}
                                    passHref
                                  >
                                    <a
