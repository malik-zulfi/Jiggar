...
export default function CvDatabasePage() {
  ...
  const [selectedCvEmails, setSelectedCvEmails] = useState<Set<string>>(
    new Set()
  );
    ...
    const filtered = searchTerm.trim()
      ? cvDatabase.filter(
          (cv) =>
            ...
            cv.structuredContent.skills?.some((s) =>
      ...
      session.candidates.forEach((candidate) => {
        ...
        if (!email) {
          const dbRecord = cvDatabase.find(
            (cv) =>
              cv.name.toLowerCase() ===
              candidate.analysis.candidateName.toLowerCase()
          );
          if (dbRecord) {
  ...
  const resolveConflict = (action: "replace" | "skip") => {
    ...
    if (action === "replace") {
      ...
      setCvDatabase((prevDb) => {
        ...
        return Array.from(dbMap.values()).sort(
          (a, b) =>
      ...
      toast({
        description: `Upload for ${currentConflict.newRecord.name} was skipped.`,
      });
    }
  ...
  const runRelevanceCheck = useCallback(
    async (candidatesToCheck: CvDatabaseRecord[]) => {
      ...
      toast({
        description: `Checking for suitable positions for ${candidatesToCheck.length} candidate(s)...`,
      });
        ...
        if (allNewPositions.length > 0) {
          setSuitablePositions((prev) => {
            const existingSet = new Set(
              prev.map((p) => `${p.candidateEmail}-${p.assessment.id}`)
            );
            const uniqueNewPositions = allNewPositions.filter(
              (p) => !existingSet.has(`${p.candidateEmail}-${p.assessment.id}`)
            );
          ...
          toast({
            description: `No new relevant positions found for the selected candidate(s).`,
          });
        }
      } catch (error: any) {
  ...
  const handleNewCandidatesAdded = useCallback(
    (newCandidates: CvDatabaseRecord[]) => {
  ...
  const handleProcessCvs = useCallback(async () => {
    ...
    if (successCount > 0) {
      toast({
        description: `${successCount} new CV(s) processed and added to the database.`,
      });
    }
  ...
  const handleDeleteCv = (emailsToDelete: string[]) => {
    ...
    const emailsToDeleteSet = new Set(
      emailsToDelete.map((e) => e.toLowerCase())
    );

    setCvDatabase((prev) =>
      prev.filter((cv) => !emailsToDeleteSet.has(cv.email.toLowerCase()))
    );
    ...
    setHistory((prevHistory) => {
      return prevHistory.map((session) => {
        ...
        return {
          ...
          summary: updatedCandidates.length > 0 ? session.summary : null,
        };
      });
    });
  ...
  const handleAddFromPopover = useCallback(
    async (
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50/50">
      ...
      <main className="flex-1 p-4 md:p-6">
        <div className="container mx-auto space-y-6">
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                <FileUp /> Add New Candidates
              </CardTitle>
              <CardDescription>
                Upload CVs and tag them with a job code to add them to the
                central database. If a candidate&apos;s email already exists,
                you will be asked to confirm the replacement.
              </CardDescription>
            </CardHeader>
              ...
              {Object.keys(processingStatus).length > 0 && (
                <div className="mt-4">
                  <ProgressLoader
                    ...
                    statusList={Object.values(processingStatus)}
                  />
                </div>
              )}
            </CardContent>
            <CardFooter>
              <Button
                ...
                disabled={
                  (cvsToUpload.length === 0 || !jobCode) && !isProcessing
                }
                size="lg"
              >
                {isProcessing ? (
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                <Database /> Candidate Records ({cvDatabase.length})
              </CardTitle>
              <div className="flex justify-between items-center gap-4 pt-2">
                <CardDescription>
                  Browse, search, and manage all candidates in the database.
                </CardDescription>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          Experience <ArrowUpDown className="h-3 w-3" />
                        </div>
                      </TableHead>
                    ...
                    {sortedAndFilteredCvs.length > 0 ? (
                      sortedAndFilteredCvs.map((cv) => {
                        const candidateAssessments =
                          assessmentMap.get(cv.email.toLowerCase()) || [];
                          ...
                          >
                            <TableCell className="px-4">
                              <Checkbox
                                ...
                                onCheckedChange={(checked) =>
                                  handleSelectOne(cv.email, !!checked)
                                }
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        ...
                                        >
                                          ...
                                          {count > 0 && (
                                            <sup className="font-bold text-xs -ml-1 text-muted-foreground">
                                              {count}
                                            </sup>
                                          )}
                                        </div>
                                  ...
                                  <PopoverContent className="w-96 p-2">
                                    <div className="space-y-1 mb-2 p-2">
                                      <h4 className="font-medium leading-none">
                                        Assessments for {cv.name}
                                      </h4>
                                      <p className="text-sm text-muted-foreground">
                                        Quick links to view assessments.
                                      </p>
                                    </div>
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => {
          if (!isOpen) setCurrentConflict(null);
        }}
      >
        {currentConflict && (
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <AlertTriangle className="text-amber-500" /> Replace Existing
                Candidate?
              </DialogTitle>
              <DialogDescription>
      ...
      <PopoverContent className="w-80 p-2">
        <div className="grid gap-4">
          <div className="space-y-1">
            <h4 className="font-medium leading-none">
              Add <span className="text-primary">{candidate.name}</span> to...
            </h4>
            <p className="text-sm text-muted-foreground">
            ...
            {compatibleAssessments.length > 0 ? (
              compatibleAssessments.map((session) => (
                <button
                  key={session.id}
                  onClick={() =>
                    onAdd(candidate, session, () => setIsOpen(false))
                  }
              ...
              <p className="p-2 text-sm text-center text-muted-foreground">
                No compatible unassessed positions found.
              </p>
            )}
          </div>
        </div>
...
const BulkActions = ({
  ...
  const commonAssessments = useMemo(() => {
    ...
    return assessments.filter((session) => {
      ...
      return !selectedCandidates.some((sel) =>
        sessionEmails.has(sel.email.toLowerCase())
      );
    });
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline">
            <PlusCircle className="mr-2 h-4 w-4" /> Add to Assessment
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-80 p-2">
          <div className="grid gap-4">
            <div className="space-y-1">
              <h4 className="font-medium leading-none">
                Add {selectedCount} Candidates to...
              </h4>
              <p className="text-sm text-muted-foreground">
