...
interface AppContextType {
  ...
  suitablePositions: SuitablePosition[];
  setSuitablePositions: React.Dispatch<
    React.SetStateAction<SuitablePosition[]>
  >;
...
export function useAppContext() {
  ...
  if (!context) {
    throw new Error(
      'useAppContext must be used within an AppProvider. It seems the context is not available.'
    );
  }
  return context;
}
...
export function ClientProvider({
  ...
  const [suitablePositions, setSuitablePositions] = useState<
    SuitablePosition[]
  >([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedSuitablePositions = localStorage.getItem(
        SUITABLE_POSITIONS_KEY
      );
  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      ...
      localStorage.setItem(
        SUITABLE_POSITIONS_KEY,
        JSON.stringify(suitablePositions)
      );
    }
  }, [history, cvDatabase, suitablePositions, isLoading]);

  const handleBulkImport = useCallback(
    (data: ImportedData, mode: ImportMode) => {
      if (mode === 'replace') {
        ...
        setTimeout(() => {
          toast({
            ...
            description: 'All data has been replaced.',
          });
        }, 0);
        return;
      }
      ...
      if (mode === 'append') {
        ...
        setCvDatabase((prev) => {
          ...
          const newCvs = data.cvDatabase.filter(
        ...
        setSuitablePositions((prev) => {
          ...
          const newPositions = data.suitablePositions.filter(
        ...
        setTimeout(() => {
          toast({
            title: 'Import Complete',
            description: 'New data has been appended.',
          });
        }, 0);
      }
    },
  ...
  const runGlobalRelevanceCheck = useCallback(async () => {
    ...
    setTimeout(() => {
      toast({
        description: `Checking for suitable positions for all ${cvDatabase.length} candidate(s)...`,
      });
    }, 0);
