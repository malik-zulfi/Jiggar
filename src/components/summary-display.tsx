      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            <div className="flex-1">
                <CardTitle className="text-lg">Overall Assessment Summary</CardTitle>
      ...
      <CardContent className="p-6">
        <Tabs defaultValue="tiers" className="w-full">
          ...
          <TabsContent value="strategy" className="mt-6">
             <h4 className="font-semibold mb-3 text-lg">Suggested Interview Strategy</h4>
...
const TierList = ({ title, candidates, icon }: { title: string; candidates: string[]; icon: React.ReactNode }) => (
  <div className="p-4 rounded-lg border bg-card shadow-sm">
    <h4 className="font-semibold mb-4 flex items-center text-base">{icon}<span className="ml-2">{title}</span></h4>
    {candidates.length > 0 ? (
      <ul className="space-y-2 text-sm">
        {candidates.map((name, i) => <li key={i} className="p-2 rounded-md bg-muted/50">{name}</li>)}
      </ul>
    ) : (
