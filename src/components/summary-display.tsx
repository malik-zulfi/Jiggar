...
function extractAllRequirements(jd: ExtractJDCriteriaOutput): Requirement[] {
  ...
  const addGroupedReqs = (
    category:
      | { MUST_HAVE: RequirementGroup[]; NICE_TO_HAVE: RequirementGroup[] }
      | undefined
  ) => {
    if (category) {
      category.MUST_HAVE.forEach((group) =>
        allReqs.push(...group.requirements)
      );
      category.NICE_TO_HAVE.forEach((group) =>
        allReqs.push(...group.requirements)
      );
    }
  };
    ...
    try {
      ...
      if (allJdRequirements.length === 0) {
        toast({
          ...
          title: 'Export Failed',
          description:
            'No job description requirements to create a report from.',
        });
      ...
      const dataRows = allJdRequirements.map((jdReq) => {
        ...
        candidateNames.forEach((name) => {
          ...
          const alignmentDetail = candidate?.alignmentDetails.find(
            (detail) =>
              normalizeString(detail.requirement).substring(0, 50) ===
              normalizeString(jdReq.description).substring(0, 50)
          );
      ...
      candidateNames.forEach((name, index) => {
        ...
        const combinedWidth =
          colWidths[colIndex].wch! + colWidths[colIndex + 1].wch!;
        if (nameLength > combinedWidth) {
          colWidths[colIndex + 1].wch =
            nameLength - colWidths[colIndex].wch! + 2;
        }
      });
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
          <div className="flex-1">
            ...
            <CardDescription>
              A complete overview of all candidates and strategic
              recommendations.
            </CardDescription>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <Button
              ...
              disabled={isExportingExcel || isExporting}
              variant="outline"
            >
              {isExportingExcel ? (
      ...
      <CardContent className="p-6">
        <Tabs defaultValue="tiers" className="w-full">
          <TabsList className="grid w-full grid-cols-1 md:grid-cols-3">
            ...
            <TabsTrigger value="commonalities">
          ...
          <TabsContent value="tiers" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <TierList
                title="Top Tier"
                candidates={summary.topTier}
                icon={<Award className="h-5 w-5 text-accent" />}
              />
              <TierList
                title="Mid Tier"
                candidates={summary.midTier}
                icon={<UserCheck className="h-5 w-5 text-primary" />}
              />
              <TierList
                title="Not Suitable"
                candidates={summary.notSuitable}
          ...
          <TabsContent value="commonalities" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h4 className="font-semibold mb-3 text-lg">Common Strengths</h4>
              ...
              <div>
                <h4 className="font-semibold mb-3 text-lg">Common Gaps</h4>
                <ul className="list-disc list-outside pl-5 space-y-2 text-sm">
