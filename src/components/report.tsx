          ...
          if ('groupType' in req) {
            return (
              <li key={index} className="text-gray-700">
                {req.requirements
                  .map((r) => r.description)
                  .join(` ${req.groupType === 'ANY' ? 'OR' : 'AND'} `)}
                <span className="text-sm font-semibold">
                  {' '}
                  ({req.requirements[0].priority.replace('-', ' ')})
                </span>
              </li>
            );
          }
      ...
      <table className="w-full text-left border-collapse text-sm border-t border-gray-200">
        <thead>
          <tr className="break-inside-avoid">
            ...
            <th className="p-2 bg-gray-100 font-bold w-[15%] text-center">
              Status
            </th>
          </tr>
        </thead>
            ...
            return (
              <React.Fragment key={category}>
                <tr className="break-inside-avoid bg-gray-50">
                  <td
                    colSpan={3}
                    className="p-2 font-bold text-gray-800 border-t border-b border-gray-200"
                  >
                    {category}
                  </td>
                </tr>
                {items.map((item, index) => {
                  const info =
                    statusInfo[item.status] || statusInfo['Not Mentioned'];
                  return (
                    <tr
                      key={index}
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          'p-2 align-top font-bold text-center text-lg',
                          info.className
                        )}
                      >
                        {info.text}
...
export default function Report({
  ...
  const allEducationReqs = [
    ...Requirements.Education.MUST_HAVE,
    ...Requirements.Education.NICE_TO_HAVE,
  ];
  const allCertificationReqs = [
    ...Requirements.Certifications.MUST_HAVE,
    ...Requirements.Certifications.NICE_TO_HAVE,
  ];
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600">
          A complete overview of all candidates and strategic recommendations.
        </p>
      </div>

      <div className="mb-8 p-4 border border-gray-200 rounded-lg break-inside-avoid">
        <h2 className="text-2xl font-bold mb-1 text-gray-800">
          {analyzedJd.JobTitle || 'Job Description Breakdown'}
        </h2>
        {analyzedJd.PositionNumber && (
          <p className="text-md text-gray-600 mb-4">
            Position #{analyzedJd.PositionNumber}
          </p>
        )}
        <div className="columns-2 gap-8">
          ...
          <RequirementList
            title="Experience"
            requirements={Requirements.Experience.NICE_TO_HAVE}
          />
          <RequirementList
            title="Certifications"
            requirements={allCertificationReqs}
          />
          <RequirementList
            title="Technical Skills"
            requirements={[
              ...Requirements.TechnicalSkills.MUST_HAVE,
              ...Requirements.TechnicalSkills.NICE_TO_HAVE,
            ]}
          />
          <RequirementList
            title="Soft Skills"
            requirements={[
              ...Requirements.SoftSkills.MUST_HAVE,
              ...Requirements.SoftSkills.NICE_TO_HAVE,
            ]}
          />
          <RequirementList
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              {summary.topTier.length > 0 ? (
                summary.topTier.map((c) => (
                  <p key={c} className="p-1 bg-green-50 rounded-sm text-sm">
                    {c}
                  </p>
                ))
              ) : (
            ...
            <div>
              ...
              {summary.midTier.length > 0 ? (
                summary.midTier.map((c) => (
                  <p key={c} className="p-1 bg-yellow-50 rounded-sm text-sm">
                    {c}
                  </p>
                ))
              ) : (
            ...
            <div>
              ...
              {summary.notSuitable.length > 0 ? (
                summary.notSuitable.map((c) => (
                  <p key={c} className="p-1 bg-red-50 rounded-sm text-sm">
                    {c}
                  </p>
                ))
              ) : (
      ...
      <div>
        ...
        {candidates.map((candidate, index) => (
          <div
            ...
            className="mb-6 p-4 border border-gray-200 rounded-lg break-inside-avoid"
          >
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-2xl font-bold">{candidate.candidateName}</h3>
              <div className="flex flex-col items-end gap-2 text-right">
                <span
                  className={`px-3 py-1 text-sm font-semibold rounded-full ${getRecommendationInfo(candidate.recommendation)}`}
                >
                  {candidate.recommendation}
                </span>
                <span
                  className={`px-3 py-1 text-sm font-semibold rounded-full ${getScoreInfo(candidate.alignmentScore)}`}
                >
