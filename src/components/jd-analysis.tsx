...
const RequirementItem = ({
  ...
  const isModified =
    item.priority !== item.originalPriority ||
  ...
  const handleToggle = () => {
    const newPriority =
      item.priority === 'MUST_HAVE' ? 'NICE_TO_HAVE' : 'MUST_HAVE';
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 flex-1">
        ...
        <span className={cn('flex-1', !isModified && 'ml-6')}>
          {item.description}
        </span>
      </div>
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent>
              <p>
                Toggle to{' '}
                {item.priority === 'MUST_HAVE' ? 'Nice to Have' : 'Must Have'}
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
...
const RequirementSection = ({
  ...
  const renderList = (
    items: (Requirement | RequirementGroup)[],
    priority: Priority
  ) => (
      ...
      {items.map((item, index) => {
        if (isRequirementGroup(item)) {
          return (
            <li
              key={`group-${index}`}
              className="p-2 border rounded-md bg-background/30"
            >
              <Badge variant="secondary" className="mb-2">
                Must match {item.groupType === 'ANY' ? 'ANY' : 'ALL'} of:
              </Badge>
              <ul className="space-y-1">
                {item.requirements.map((req) => (
                  <RequirementItem
                    key={`${req.id}-${req.priority}`}
                    item={req}
                    category={category}
...
const AddRequirementForm = ({
  ...
  onAdd: (description: string, priority: Priority, score: number) => void;
}) => {
        ...
        <div className="grid gap-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              This will be added to the &apos;Additional Requirements&apos;
              category.
            </p>
          </div>
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              ...
              >
                <div>
                  ...
                  <Label htmlFor="r-must" className="ml-2">
                    Must Have
                  </Label>
                </div>
                <div>
                  <RadioGroupItem value="NICE_TO_HAVE" id="r-nice" />
                  <Label htmlFor="r-nice" className="ml-2">
                    Nice to Have
                  </Label>
                </div>
              </RadioGroup>
...
export default function JdAnalysis({
  ...
  const experienceMustHaves: Requirement[] =
    Requirements.Experience.MUST_HAVE.Years &&
    Requirements.Experience.MUST_HAVE.Years !== 'Not Found' &&
    Requirements.Experience.MUST_HAVE.Fields.length > 0
      ? [
          {
            id: 'exp-must-years',
      ...
      <Card className="shadow-sm">
              ...
              <CollapsibleTrigger asChild>
                <div className="cursor-pointer">
                  <CardTitle className="flex items-center gap-3 text-lg">
                    ...
                    <span className="mr-2">
                      {JobTitle || 'Job Description Breakdown'}
                    </span>
                  </CardTitle>
                  <CardDescription className="mt-1">
                    The JD has been deconstructed. You can edit requirement
                    priorities and scores below.
                  </CardDescription>
                </div>
              </CollapsibleTrigger>
                ...
                <TooltipTrigger asChild>
                  <CollapsibleTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="flex-shrink-0 w-8 h-8 -mt-1 -mr-2"
                    >
        ...
        <CollapsibleContent>
          <CardContent className="p-6">
            <Collapsible
              open={objectiveVisible}
              onOpenChange={setObjectiveVisible}
              className="mb-6"
            >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    {OrganizationalRelationship.ReportsTo.length > 0 && (
                      <div>
                        ...
                        <ul className="list-disc list-outside pl-5 text-muted-foreground">
                          {OrganizationalRelationship.ReportsTo.map(
                            (role, i) => (
                              <li key={`report-${i}`}>{role}</li>
                            )
                          )}
                        </ul>
                      </div>
                    ...
                    {OrganizationalRelationship.InterfacesWith.length > 0 && (
                      <div>
                        ...
                        <ul className="list-disc list-outside pl-5 text-muted-foreground">
                          {OrganizationalRelationship.InterfacesWith.map(
                            (role, i) => (
                              <li key={`interface-${i}`}>{role}</li>
                            )
                          )}
                        </ul>
                      </div>
              ...
              <CollapsibleTrigger asChild>
                <Button
                  variant="link"
                  className="px-0 h-auto text-xs mt-2 -mb-4"
                >
                  {objectiveVisible ? (
                  ...
                  {objectiveVisible
                    ? 'Hide Objective & Org Chart'
                    : 'Show Objective & Org Chart'}
                </Button>
              </CollapsibleTrigger>
            </Collapsible>
            ...
            <div className="md:columns-2 gap-8 space-y-6">
              <RequirementSection
                ...
                mustHaves={Requirements.Education.MUST_HAVE}
                niceToHaves={Requirements.Education.NICE_TO_HAVE}
                category="Education"
                onRequirementChange={onRequirementChange}
                onScoreChange={onScoreChange}
                onDeleteRequirement={onDeleteRequirement}
              ...
              <RequirementSection
                ...
                mustHaves={Requirements.Certifications.MUST_HAVE}
                niceToHaves={Requirements.Certifications.NICE_TO_HAVE}
              ...
              <RequirementSection
                ...
                mustHaves={experienceMustHaves}
                niceToHaves={experienceNiceToHaves}
                category="Experience"
                onRequirementChange={onRequirementChange}
                onScoreChange={onScoreChange}
                onDeleteRequirement={onDeleteRequirement}
              ...
              <RequirementSection
                ...
                mustHaves={Requirements.TechnicalSkills.MUST_HAVE}
                niceToHaves={Requirements.TechnicalSkills.NICE_TO_HAVE}
              ...
              <RequirementSection
                ...
                mustHaves={Requirements.SoftSkills.MUST_HAVE}
                niceToHaves={Requirements.SoftSkills.NICE_TO_HAVE}
                category="SoftSkills"
                onRequirementChange={onRequirementChange}
                onScoreChange={onScoreChange}
                onDeleteRequirement={onDeleteRequirement}
              ...
              <RequirementSection
                ...
                mustHaves={Responsibilities.MUST_HAVE}
                niceToHaves={Responsibilities.NICE_TO_HAVE}
              ...
              <RequirementSection
                ...
                mustHaves={Requirements.AdditionalRequirements?.MUST_HAVE}
                niceToHaves={Requirements.AdditionalRequirements?.NICE_TO_HAVE}
