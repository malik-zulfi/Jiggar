...
export function Header({ activePage, onQuickAdd }: HeaderProps) {
  const { toast } = useToast();
  const pathname = usePathname();
  const importInputRef = useRef<HTMLInputElement>(null);
  const { suitablePositions, runGlobalRelevanceCheck, manualCheckStatus } =
    useAppContext();
    ...
    reader.onload = (e) => {
      try {
        ...
        const validatedData: ImportedData = {
          ...
          suitablePositions: Array.isArray(data.suitablePositions)
        ...
        toast({
          variant: 'destructive',
          title: 'Import Failed',
          description: 'The file is not a valid JSON backup.',
        });
      }
    };
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link href={item.href}>
                      <Button
                        variant={
                          currentPage === item.page ? 'default' : 'ghost'
                        }
                        size="sm"
                        className={cn(
                          'rounded-full px-4',
                          currentPage === item.page && 'shadow-sm'
                        )}
                      >
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="rounded-md border p-4 space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Manually run AI checks across your entire database. This
                    ...
                    <div className="rounded-md border p-4 space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Export your data to a file to move it to another
                        computer, or import a previous backup.
                      </p>
