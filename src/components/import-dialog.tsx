...
export default function ImportDialog({
  ...
  const { history, cvDatabase, suitablePositions, handleBulkImport } =
    useAppContext();
  const [step, setStep] = useState<'options' | 'conflicts' | 'summary'>(
    'options'
  );
  ...
  const summaryData = useMemo(() => {
    ...
    if (importMode === 'replace') {
      return {
        ...
        candidates: {
          new: importedData.cvDatabase.length,
          replaced: 0,
          kept: 0,
        },
      };
    ...
    // Append mode
    const newAssessments = importedData.history.filter(
      (s) => !conflicts.some((c) => c.type === 'assessment' && c.id === s.id)
    );
    const assessmentsToReplace = conflicts.filter(
      (c) => c.type === 'assessment' && c.resolution === 'replace'
    ).length;
    const assessmentsToKeep = conflicts.filter(
      (c) => c.type === 'assessment' && c.resolution === 'keep'
    ).length;

    const newCandidates = importedData.cvDatabase.filter(
      (c) =>
        !conflicts.some(
          (conflict) => conflict.type === 'candidate' && conflict.id === c.email
        )
    );
    const candidatesToReplace = conflicts.filter(
      (c) => c.type === 'candidate' && c.resolution === 'replace'
    ).length;
    const candidatesToKeep = conflicts.filter(
      (c) => c.type === 'candidate' && c.resolution === 'keep'
    ).length;

    return {
      assessments: {
        new: newAssessments.length,
        replaced: assessmentsToReplace,
        kept: assessmentsToKeep,
      },
      candidates: {
        new: newCandidates.length,
        replaced: candidatesToReplace,
        kept: candidatesToKeep,
      },
    };
  ...
  const handleNext = () => {
    if (importMode === 'append') {
      const allConflicts = [
        ...conflictsAnalysis.assessmentConflicts,
        ...conflictsAnalysis.cvConflicts,
      ];
    ...
    conflicts.forEach((conflict) => {
      if (conflict.resolution === 'replace') {
        if (conflict.type === 'assessment') {
          ...
          if (index !== -1)
            finalHistory[index] = conflict.incoming as AssessmentSession;
        } else {
          const index = finalCvDatabase.findIndex(
            (c) => c.email === conflict.id
          );
          if (index !== -1)
            finalCvDatabase[index] = conflict.incoming as CvDatabaseRecord;
        }
      }
    });

    // Add new items
    const newHistory = importedData.history.filter(
      (s) => !history.some((e) => e.id === s.id)
    );
    const newCvs = importedData.cvDatabase.filter(
      (c) => !cvDatabase.some((e) => e.email === c.email)
    );
  ...
  const handleConfirmImport = () => {
    ...
    if (importMode === 'replace') {
      ...
      conflicts.forEach((conflict) => {
        if (conflict.resolution === 'replace') {
          if (conflict.type === 'assessment') {
            finalHistoryMap.set(
              conflict.id,
              conflict.incoming as AssessmentSession
            );
          } else {
      ...
      const existingKeys = new Set(
        suitablePositions.map((p) => `${p.candidateEmail}-${p.assessment.id}`)
      );
      const newPositions =
        importedData?.suitablePositions.filter(
          (p) => !existingKeys.has(`${p.candidateEmail}-${p.assessment.id}`)
        ) || [];
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg md:max-w-xl lg:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'options' &&
              'Choose how to import the data from your backup file.'}
            {step === 'conflicts' &&
              'Some items in your file conflict with existing data. Choose how to handle them.'}
            {step === 'summary' &&
          ...
          <div>
            ...
            >
              <Label
                htmlFor="mode-append"
                className={cn(
                  'p-4 border rounded-md cursor-pointer hover:border-primary',
                  importMode === 'append' &&
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  Adds new assessments and candidates. If any conflicts are
                  found, you will be asked how to handle them.
                </p>
              </Label>
              <Label
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  Deletes all current data and replaces it with the content from
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={() =>
                              handleConflictResolution(conflict.id, 'keep')
                            }
                          >
                            Keep Existing
                          </Button>
                          <Button
                            ...
                            onClick={() =>
                              handleConflictResolution(conflict.id, 'replace')
                            }
                          >
                            Replace
                          </Button>
        ...
        {step === 'summary' && summaryData && (
          <div className="space-y-4">
            <h4 className="font-semibold">
              Import Summary (
              {importMode === 'replace' ? 'Replacing All' : 'Appending'})
            </h4>
            <div className="grid grid-cols-2 gap-4">
              <Card>
                <CardContent className="p-4 space-y-2">
                  <h5 className="font-medium text-sm flex items-center gap-2">
                    <GanttChartSquare className="text-primary" /> Assessments
                  </h5>
                  <p className="text-xs flex justify-between">
                    New:{' '}
                    <span className="font-bold">
                      {summaryData.assessments.new}
                    </span>
                  </p>
                  <p className="text-xs flex justify-between">
                    Replaced:{' '}
                    <span className="font-bold">
                      {summaryData.assessments.replaced}
                    </span>
                  </p>
                  <p className="text-xs flex justify-between">
                    Kept Existing:{' '}
                    <span className="font-bold">
                      {summaryData.assessments.kept}
                    </span>
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 space-y-2">
                  <h5 className="font-medium text-sm flex items-center gap-2">
                    <Database className="text-primary" /> Candidates
                  </h5>
                  <p className="text-xs flex justify-between">
                    New:{' '}
                    <span className="font-bold">
                      {summaryData.candidates.new}
                    </span>
                  </p>
                  <p className="text-xs flex justify-between">
                    Replaced:{' '}
                    <span className="font-bold">
                      {summaryData.candidates.replaced}
                    </span>
                  </p>
                  <p className="text-xs flex justify-between">
                    Kept Existing:{' '}
                    <span className="font-bold">
                      {summaryData.candidates.kept}
                    </span>
                  </p>
                </CardContent>
              </Card>
          ...
          {step === 'summary' && (
            <>
              <Button
                variant="ghost"
                onClick={() =>
                  setStep(conflicts.length > 0 ? 'conflicts' : 'options')
                }
              >
                <Undo2 className="mr-2 h-4 w-4" /> Back
              </Button>
